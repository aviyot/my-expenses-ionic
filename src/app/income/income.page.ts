...
export class IncomePage implements OnInit {
  ...
  onSave() {
    if (this.incomeGroup.valid) {
      const { type, amount, from, startDate } = this.incomeGroup.value;
  ...
  onDelete() {
    this.totalIncomes -= this.incomes.find(
      (income) => income.id === this.selectedIncomeId
    ).amount;
  ...
  openAddForm(type: boolean) {
    this.newAdd = type;
    if (!this.newAdd) this.formIsValid = false;
  }
  ngOnDestroy() {
